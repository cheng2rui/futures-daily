...
def check() -> None:
    ...
    try:
        ...
        db.add(CrawlerRun(trade_date=trade_date, exchange="DCE", kind="seat_rank", source="akshare", status="failed", rows=0, saved=0, error="DCE seat rank fallback unavailable"))
        db.add(DataGap(trade_date=trade_date, exchange="DCE", kind="seat_rank", severity="error", status="open", message="DCE seat rank fallback unavailable"))
        db.commit()
        ...
        assert any(issue["kind"] == "seat_rank" for issue in dce["issues"])
