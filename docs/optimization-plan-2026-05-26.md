...
- 新增 `app/services/dce_browser_parser.py`：DCE HTML table candidate parser dry-run。
- 根据表头推断 rank/party/成交/持买/持卖/增减列，尝试转换为标准化 seat-like rows；结果通过 `parser_dry_run` 随 browser probe replay 返回。
- 仍不写 `seat_rank_rows`，只用于验证 official browser table 是否具备可入库 parser 条件。

## v0.5.23 执行记录

- 数据诊断页 Replay 结果新增 DCE Parser Dry-run 可视化区。
- 直接展示 parser dry-run 状态、尝试表格数、解析行数、错误数、列映射、样例行和错误详情。
- 方便从 Browser Probe → Raw Archive → Replay → Parser dry-run 在前端完成闭环审查。

## v0.5.24 执行记录

- 新增 `app/services/parser_promotion_guard.py`：parser dry-run 提升门禁。
- Guard 检查最少解析行数、成功率、错误率、必要列映射（rank/party）和至少一个数值列映射。
- Browser probe replay 返回 `promotion_guard`，数据诊断页展示 PASS/BLOCKED、指标、阻断原因和下一步动作。
- 当前仍为 advisory guard，不开放正式入库。

## v0.5.25 执行记录

- 新增 `app/services/parser_promotion_preview.py` 和 `POST /api/dataset/raw-archives/{file_id}/promotion-preview`。
- Promotion preview 先跑 replay + promotion guard；只有 guard PASS 时才返回将来可能写入 `seat_rank_rows` 的预览行。
- 当前 `would_write=false`，不做任何数据库写入；guard BLOCKED 时不返回预览行。

## v0.5.26 执行记录

- Raw Archive 表格中对 `*_browser_probe` 归档新增 `Preview` 按钮。
- 数据诊断页接入 `promotion-preview` API，展示 preview status、preview rows、guard 决策和阻断原因。
- Preview 明确 `would_write=false`，继续保持不写库。

## v0.5.27 执行记录

- Promotion preview 行新增稳定 `record_id`、`source_file_id`、`source` 和真实 trade_date/exchange。
- `record_id` 基于 canonical tuple hash，便于后续正式入库前做稳定对比、去重和审计。
- 仍保持 `would_write=false`，只预览不写库。

## v0.5.28 执行记录

- Promotion Preview 前端新增行级表格，不再只能查看 JSON。
- 表格展示 record_id、rank、席位、成交/成交变化、多头持仓/变化、空头持仓/变化。
- 保留原始 JSON details，便于审计。

## v0.5.29 执行记录

- Promotion Preview 通过 Guard 后，前端“确认入库”会调用 `promotion-apply`。
- 接口要求 `PROMOTE-<raw_archive_id>` 确认令牌，按精确席位行去重写入 `seat_rank_rows`，不清空已有席位数据，并记录 crawler run / resolved gap。

## v0.5.30 执行记录

- 日报生成增加 no-data gate：交易日没有可用日行情时，报告状态标为 `blocked`，综合分置 0，并明确提示先执行数据诊断/自动补采，不再把空数据渲染成“冰点行情”。
- `/reports/latest` 增加 `meta.latest_state`：当最新采集/诊断活动晚于最新已生成日报时，返回 `stale` 状态和下一步提示，避免旧日报被误认为最新日报。
- Retry Runner 每次执行后都会做 finalization：materialize 数据集、同步 coverage gaps、重建日报或生成 blocked report，并把 finalization 写入 JobRun，形成“补采 → 物化 → 报告状态”的闭环。
- 今日看板顶部增加 stale warning，能直接看到“最新活动日期已晚于当前日报”。

## v0.5.31 执行记录

- Run Records 升级为 current-state 语义：根据 coverage matrix + report status 计算 `complete` / `partial` / `error`，并输出标准 `run_summary`。
- `generate_report` 与 Retry Runner finalization 都会写入 `run_summary`，包含 run_id、profile、状态和覆盖计数；Retry Runner 的 finalization 继续作为补采后是否可提升当前日报状态的依据。
- `/reports/latest` 增加 `meta.current_state`，展示当前日报来源 run 的状态、run_id、profile、覆盖计数与提示；前端今日看板可显示 current-state/run 信息。

## v0.5.32 执行记录

- 指定日期 `/reports/{trade_date}` 也会注入 `meta.current_state`，不再只有 `/reports/latest` 能看到 current-state。
- 数据诊断页新增“当前日报状态”卡片，展示 run_id、profile、complete/partial/error、核心/综合覆盖与结束时间。
- 运行记录页识别 JobRun.result_json 中的 `run_summary`，列表详情直接显示 Current State、run_id、profile 和覆盖计数，方便追溯“当前日报来自哪次 run”。

## v0.5.33 执行记录

- Retry Planner 不再把 `archive_signal` 固定跳过：当 rsstsx 结构化归档存在时，会生成 `materialize_archive_signal` 可执行步骤，source=`rsstsx_archive`。
- Retry Runner 支持 `materialize_archive_signal`：只读本地 rsstsx 归档，重建 variety dataset / DailyCoverage，并按需重建日报，不访问外部站点、不伪造缺失品种。
- 运行记录页补齐 `archive_signal` / `materialize_archive_signal` 标签，自动补采历史能看懂席位归档物化步骤。

## v0.5.34 执行记录

- `materialize_archive_signal` 执行结果新增 `archive_effect`：记录 rsstsx 归档品种数、目标交易所、物化结构信号覆盖率、Top 净变化与 Focus5 样例。
- Retry History 序列化保留每步 `archive_effect`，数据诊断页“最近一次操作结果”和自动补采历史能直接展示席位归档物化带来的覆盖与代表信号。
- 数据诊断页支持单步席位归档物化入口，并在操作结果中展示 rsstsx 归档覆盖、来源和 Top 净变化标签。
