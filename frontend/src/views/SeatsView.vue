  ...
  <div class="seats">
    ...
    <div class="filters">
      ...
      <select v-model="filters.exchange">
        ...
        <option>DCE</option><option>CZCE</option><option>SHFE</option><option>CFFEX</option><option>GFEX</option><option>INE</option>
      </select>
...
const watchRows = computed(() => (report.value.seats?.watchlist || []).map(x => [x.exchange, x.variety, x.contract, x.long_party_name || '-', x.long_open_interest_chg ?? '-', x.short_party_name || '-', x.short_open_interest_chg ?? '-']))
const detailRows = computed(() => details.value.map(x => [x.trade_date, x.exchange, x.variety, x.rank, x.long_party_name || '-', x.long_open_interest_chg ?? '-', x.short_party_name || '-', x.short_open_interest_chg ?? '-']))
const trendRows = computed(() => (archiveHistory.value.trend || []).map(x => [formatDate(x.date), x.seat, x.variety, x.exchange, fmtSigned(x.netDelta), fmtSigned(x.netVol)]))
...
function row(x) { return [x.exchange, x.variety, x.contract, x.seat, x.value ?? '-', x.change ?? '-'] }
function archiveRow(x) { return [x.displayName || x.name, x.exchange, x.longShortRatio ?? '-', fmtSigned(x.netDelta), x.netDir || '-', `${x.longCR5 ?? '-'}%`, `${x.shortCR5 ?? '-'}%`] }
